...
from .utils import staple, dice_score, norm, Visual_tensor,std
...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        if isinstance(model_output, tuple):
            model_output, cal, mid_feature = model_output
    ...
    def p_sample_loop_known(
        ...
        if self.dpm_solver:
            ...
            # sample[:,-1,:,:] = std(sample[:,-1,:,:])
            sample = torch.clamp(sample[:,-1,:,:].unsqueeze(1),0,1)
            sample = std(norm(sample))
            # final["sample"] = torch.where(sample<1,0,1)
            final["sample"] = sample
            # final["cal"] = cal
            # temp_out = torch.clamp(final["sample"][:,-1,:,:].unsqueeze(1), 0, 1)
            # Visual_tensor(input_tensor=final["sample"],indexlist=[0],transposelist=[],sublist=[1,1],batchsize=1)
            ...
            for sample in self.p_sample_loop_progressive(
                ...
                final = sample
                final["sample"] = torch.clamp(final["sample"],0,1)
                # final["sample"] = torch.where(final["sample"]>0.9,1.0,0.0)
    ...
    def training_losses_segmentation(self, model, classifier, distribution, x_start , t, model_kwargs=None, noise=None):
        """
        ...
        res = torch.where(mask < 1, 0, 1)   #merge all tumor classes into one to get a binary segmentation mask
        
       
        res_t = self.q_sample(res, t, noise=noise)     #add noise to the segmentation channel
        x_t=x_start.float()
        x_t[:, -1:, ...]=res_t.float()
        # Visual_tensor(input_tensor= res,indexlist=[0],transposelist=[],sublist=[2,3],batchsize=16)
        terms = {}
        if self.loss_type == LossType.MSE or self.loss_type == LossType.RESCALED_MSE:
            
            model_output, cal, hso_list = model(x_t, self._scale_timesteps(t), **model_kwargs)
            from torchsummary import summary
            # print(summary(model,[x_t,self._scale_timesteps(t)],depth=5,col_names=["input_size", "output_size", "num_params", "kernel_size"]))
            ...
            prior_latent_space = distribution.forward(hso_list) 
            posterior_latent_space = distribution.forward(torch.cat((patch, res), dim=1))
            
            kl_div_list = [kl.kl_divergence(prior, posterior) for prior, posterior in zip(prior_latent_space,posterior_latent_space)]

            kl_div = kl_div_list[0]*0.5 + kl_div_list[1]*0.4 + kl_div_list[2]*0.3 + kl_div_list[3]*0.2
