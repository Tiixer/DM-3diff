...
from .utils import to_cuda, maybe_to_torch, Visual_tensor
...
class FFParser(nn.Module):
    def __init__(self, dim, h=256, w=256):
        super().__init__()
        self.w = w
        self.h = h
    ...
    def forward(self, x):
        ...
        xfm = DWTForward(J=1, wave='sym3', mode='zero').cuda()
        ifm = DWTInverse(wave='sym3', mode='zero').cuda()
...
class UNetModel_v1preview(nn.Module):
    ...
    def forward(self, x, timesteps, y=None):
        ...
        for ind, module in enumerate(self.input_blocks):
            ...
            h = module(h, emb)
            hs.append(h)
        uemb, cal, hs_list = self.highway_forward(c, [hs[3],hs[6],hs[9],hs[12]]) ## hs[3,6,9,12]
        ...
        hso_list = [hso[11],hso[8],hso[5],hso[2]]
        out = self.out(h) 
        return out, cal, hso_list
...
class Generic_UNet(SegmentationNetwork):
    ...
    def __init__(self, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
        ...
        for d in range(num_pool):
            ...
            if d < num_pool -1 and self.highway:
                ...
                self.wrb_block.append(WRB(in_channels=2 **(d+5),lrl=False))
            # if d != num_pool - 1 and self.highway:
        ...
        self.wrb_block = nn.ModuleList(self.wrb_block)
    ...
    def forward(self, x, hs = None):
        ...
        return emb, seg_outputs, hs_list
